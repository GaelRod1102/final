  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(onTap: (newIndex) => setState(() => _index = newIndex), currentIndex: _index, items: [
        ...
        BottomNavigationBarItem(
            icon: Icon(Icons.directions_transit, color: Colors.greenAccent, size: 30.0), //icon
            title: Text('Surcursales')), //
        BottomNavigationBarItem(
            icon: Icon(Icons.info, color: Colors.greenAccent, size: 30.0), //icon
            title: Text('Examen')), //
      ]), // bottom
    ); //Fin scaffold
  } //Fin widget build
